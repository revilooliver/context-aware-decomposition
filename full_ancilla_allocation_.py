...
class FullAncillaAllocation_(AnalysisPass):
    """Allocate all idle nodes from the coupling map as ancilla on the layout.
    ...
    def run(self, dag):
        ...
        #access the layout set by the layout passes
        layout = self.property_set.get("layout")

        #Layout needs to be set to identify ancilla qubits (By definition ancilla qubits are those
        #qubits in the hardware to which no program qubit has been mapped. Thus, a program to hardware
        #mapping is assumed)
        if layout is None:
            raise TranspilerError('FullAncillaAllocation pass requires property_set["layout"].')

        if layout:
            FullAncillaAllocation_.validate_layout(layout.get_virtual_bits(), set(dag.qubits))
        ...
        #all qubits that are present in the hardware but are not mapped to the program are idle
