...
class ApplyLayout_(TransformationPass):
    """Transform a circuit with virtual qubits into a circuit with physical qubits.
    ...
    def run(self, dag):
        ...
        #checking if a layout exists
        if not layout:
            raise TranspilerError(
                "No 'layout' is found in property_set. Please run a Layout pass in advance."
            )
        
        #ensuring that the number of virtual qubits (including ancillae) are equal to 
        #the number of physical qubits in the device
        ...
        #creating the new dag on physical qubits
        new_dag = DAGCircuit()
        new_dag.add_qreg(q)
